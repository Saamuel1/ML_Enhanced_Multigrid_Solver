...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 10 10:57:00 2025

@author: duarteds
"""
import numpy as np
from model_lightning_case_1 import MultigridLightningSolver, TrainLossPerCycleHistory, ValLossPerCycleHistory, KDInvLogger, KDInvEarlyStopping
...
# Output directories for model and metrics
...
def run_training(N, L, nu1, nu2, batch_size, lr, max_epochs, method, nbr_iter, loss):
    ...
    k_tracker = KDInvLogger()
    kdinv_earlystop = KDInvEarlyStopping(min_delta=1e-6, patience=10, verbose=True)
    ...
    trainer = pl.Trainer(
        ...
        log_every_n_steps=1,
        callbacks=[RichProgressBar(), train_loss_tracker, val_loss_tracker, k_tracker, kdinv_earlystop],
        accelerator="auto",
    ...
    np.save(f"{save_dir_metriques}/train_losses_per_cycle_N={N}_L={L}_nbr_iter={nbr_iter}.npy", np.array(train_loss_tracker.train_losses_per_cycle))
    np.save(f"{save_dir_metriques}/val_losses_per_cycle_N={N}_L={L}_nbr_iter={nbr_iter}.npy", np.array(val_loss_tracker.val_losses_per_cycle))
